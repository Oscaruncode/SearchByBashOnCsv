...
#Functions
PRINTSECTION_FUNCT(){  #MODIFY
	echo $SECTION_RTN
        #echo SECTIONRTN + CONTENT
}
    ...
    case $SECTION in
        ${sections[0]}) #Modify if condition
            if [[ $TITLE =~ $PATRON ]]; then
                PRINTSECTION_FUNCT $SECTION_RTRN $TITLE $SUMMARY $KEYWORDS $MAIN_OBJETIVE $METHOD $RESULTS $CONCLUSIONS $RELEVANCE
            fi
            ;;
        ${sections[1]})
